...
from services.destinations_registry import POPULAR_CITIES_REGISTRY
...
def search_destination(request: Request, city: str = Query(..., description="City name to search")):
    ...
    dest = db_connection.execute_read(
        "SELECT * FROM destinations WHERE LOWER(city_name) = ?",
        (city_clean,)
    )
    ...
    dest_id = db_connection.execute_query(
        "INSERT INTO destinations (city_name, country, rating, description) VALUES (?, ?, ?, ?)",
    ...
    for place in api_data["places"]:
        db_connection.execute_query(
            "INSERT INTO tourist_places (destination_id, place_name, category, rating, latitude, longitude, address) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def get_recommendations(req: BudgetRequest):
    ...
    if dest:
        places = db_connection.execute_read(
            "SELECT DISTINCT category FROM tourist_places WHERE destination_id = ?",
    ...
    if lat and lon:
        try:
            ...
            from api.weather import WMO_WEATHER_CODES
